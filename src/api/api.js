...
export const filmsApi = {
    ...
    getFilms(filters, page){
        return instance.get(`movie?${filters.selectedTypeOfMovies.map((item)=> `field[]=typeNumber&search[]=${item}&`).join('')}${filters.genre.length ? filters.genre.map((item)=> `field[]=genres.name&search[]=${item.value}&`).join(''):''}&field[]=year&search[]=${filters.year[0]}-${filters.year[1]}&field=rating.kp&search[]=${filters.rating[0]}-${filters.rating[1]}&sortField[]=year&sortType[]=${filters.sortByRelease}&limit=30&page=${page}&field=poster&search=!null&token=${TOKEN}`)
