...
import { Description, Flex, MarginTopTitle, MarginTopValue, Name, NameTitle, TextValue, TitleDescription, ValueTextSpan} from '../FilmPage/FilmPage.styles';
...
const FilmInfoText: React.FC<FilmInfoTextProps> = (props) =>{
  const { filmInfoId, countries, genres, ratingKinopoisk, resolution } = props
    ...
    return (
       <div>
            <Name>{filmInfoId?.name} {`(${filmInfoId?.year} г.)`}</Name>
            ...
            {genres.map((i, index, arr)=>
              ...
              <MarginTopValue>
                ...
                {index===arr.length - 1
                  ?<></>
                  :<ValueTextSpan>,&nbsp;</ValueTextSpan>}</MarginTopValue>
